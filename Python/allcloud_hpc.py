...
try:
	...
	con = mdb.connect("localhost","subrata","")
	db = con.cursor()
...
def initDB():
	try:
		...
		a = str(a)
		if(a.find("test") > 0):
        		#db.execute("drop database test")
			db.execute("use test")
			db.execute('drop table image_table')
			db.execute('drop table cloud_table')
			con.commit()
		else:
			pass
...
def plotGraph():
	...
	fig = plt.figure(figsize=(13.6,7.68))
...
if SEQUENTIAL:
	...
	for i in range(0,l):
                s = str(z[i])
                if (s.find('KASPR') > -1):
                        cmd = "python3 cloud.py -i " + s + " -x " + str(RADAR_DIAMETER) + " -y " + str(RADAR_HEIGHT) + rangeCommand
                        bar.update(i)
                        os.system(cmd)
                else:
	                pass
	...
	for i in range(0,len(z_new)):
		cmd = "python3 ./cloud.py -i " + z_new[i] + " -x " + str(RADAR_DIAMETER) + " -y " + str(RADAR_HEIGHT) + rangeCommand
		cmds.append(cmd)
